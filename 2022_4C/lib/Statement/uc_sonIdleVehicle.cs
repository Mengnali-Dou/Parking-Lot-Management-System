...
{
    public partial class uc_sonIdleVehicle : UserControl
    {
        public uc_sonIdleVehicle()
        {
            InitializeComponent();
        }
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            {
                string select = $"select * from OFIdleRent where OFIRNum = '{num}'";
                ...
                {
                    string sel = $"select OFIRComment from OFIdleRent where OFIRNum = '{num}'";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                string select = $"select * from OFSIdleRent where OFIRNum = '{num}'";
                OleDbDataAdapter inst = new OleDbDataAdapter(select, database.dbConn);
                DataTable dt = new DataTable();
                inst.Fill(dt);
                if (dt.Rows.Count == 0)
                ...
                {
                    string update = $"update OFIdleRent set OFIRComment = '{comment}' where OFIRNum = '{num}'";
