...
{
    public partial class uc_ChangeThis : UserControl
    {
        public uc_ChangeThis()
        {
            InitializeComponent();
        }
        ...
        private void btnUpdateThis_Click(object sender, EventArgs e)
        {
            ...
            if (id == string.Empty)
            {
                DialogResult result1 = MessageBox.Show("请输入ID", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                OleDbDataAdapter inst = new OleDbDataAdapter($"select * from BAAdmin where AdminID = '{id}'", database.dbConn);
                DataTable dt = new DataTable();
                inst.Fill(dt);
                if (dt.Rows.Count == 0)
                {
                    DialogResult result5 = MessageBox.Show("未查询到该ID", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                string update = $"update BAAdmin set AdminPassword = '{pwd2}' where AdminID = '{id}'";
